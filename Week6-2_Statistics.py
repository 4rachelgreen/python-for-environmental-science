...
# Save outputs of linregress as variables
...
# Polynomial fit
# fit = np.polyfit(x, y, deg)   -   Returns slope and intercept 
#   fit[0] * x + fit[1]

fit = np.polyfit(x,y,1)
fit_fn = np.poly1d(fit) 
# fit_fn is now a function which takes in x and returns an estimate for y
#       fit_fn(x) = y   where x is a number or an array

plt.plot(x,y, 'yo', x, fit_fn(x), '--k')

# Correlation coefficient (y vs predicted y)
...
# use polyval(fit,x) to turn the fit into a function (returns an array with constants)
...
# typing the variable name displays a formatted table of the regression
ols_model

# model variable has lots of dot notation variables
ols_model.resid     # residuals (error for each point)
