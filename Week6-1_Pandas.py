...
#           * Yes, it is confusing, since data frames still have indices like
#             arrays...
#
...
# 1-D series data are easy... (If you only have 1-d data, it's better to use an array)
...
# directly...
df3['food'] = np.array(['fish','lettuce','apple','banana'])

df3['nums'] = pd.Series([10,20,30,40,50])

#%% Accessing index and column name lists

df.index            # Returns index range (number of rows)
df.columns          # Returns column names (index column)
df.values           # Returns data frame in an object array
df.dtypes           # Returns the column names with data types
df.describe()       # Returns statistical info about data frame

#%% Accessng/adding/deleting column data

# access the column by name: string (df['colname']) OR by dot notation (df.columnname)
...
# adding the same value to all rows (propagated value)
...
# extracting values and deleting (pop out) - extracts column from frame and deletes it
...
dates = pd.date_range('1/1/2016','12/31/2018')
...
#   row and column (must use datetime index if present, otherwise normal)
df.loc['2016-01-02':'2016-01-04',['A','B']]

#   index and columns
df.loc[dates[2],'A']    # or if you have a generic index: df.loc[index[2],'A']
...
#   only apply the equation to positive values in A
...
# Concatenate dataframes based on column names
result = pd.concat([df1, df2])

# Concatenate data frames based on an axis; keeps columns and rows separate
result2 = pd.concat([df1, df2], axis = 1)

# Add one data frame to the other using append based on column names
result3 = df1.append(df2)

# Add series to data frame using concatenate function (same as adding a column)
result4 = pd.concat([df1,series],axis=1)

# Combining data with matching indices
dates = pd.date_range('1/1/2016', periods=8)
df = pd.DataFrame(np.random.randn(8, 4), index=dates, columns=['A', 'B', 'C', 'D'])
...
# Monster statements with multiple instructions
np_with_reshape = pd.np.array(df3.A).reshape((4,4))
