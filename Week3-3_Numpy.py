...
#
# The NUMPY library is one way to generate and manipulate arrays
#
# Arrays in Numpy can be multi-dimensional
#   1-Dimension = One row of columns (like a list)
...
# CONSTRUCTING ARRAYS
# Arrays are normally constructed as individual rows
# 1-D arrays (Elements in a 1-D array are defaulted as rows in one column)
...
# 2-D Arrays
# np.array( [ [first row], [second row], [third row]...] )
...
# Array attributes (the len function does not work for 2-D arrays)
print(two_d.ndim)
print(two_d.shape)
...
# !!!   np.arange(1.0,17.0) is not the same as np.arange(1,17)
...
# linspace(start, end, num-points) ** end is inclusive, without endpoint option
b = np.linspace(0, 1, 6)

c = np.linspace(0, 1, 5, endpoint=False)

# Zeros and Ones, arrays of a stated size filled with 0 or 1 (remember double parentheses)
...
# random number arrays (useful for sampling, modeling, etc.)
#   np.random.rand()
...
#   in this example below, a is the original and b is a "View"
...
# often we need to transform array shapes
